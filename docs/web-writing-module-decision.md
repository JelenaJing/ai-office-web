...
以下是 Web 文稿 P1 代码实施清单；已实现项作为后续验收基线，未实现项继续按清单推进。

实施状态：

| 项目 | 状态 | 说明 |
| --- | --- | --- |
| P1-A：A4 样式对齐 Electron | 已进入实现 | 已修复 A4 CSS bug，并将页面宽高、边距、字体、字号、行高、页眉页脚和基础正文元素样式接入 `pageSpec` / `headerFooter` |
| P1-B：TipTap 基础扩展 | 已进入实现 | 已接入 Underline、TextAlign、Highlight、Table、TaskList、Typography、TextStyle、FontFamily、FontSize 等基础扩展 |
| P1-C：右键菜单 | 已进入实现 | 已新增 `DocumentContextMenu`、`useDocumentPatchActions` hook，已接入 `A4RichTextEditor onContextMenu`，已在 `WordLikeDocumentEditor` 集成右键 AI 动作和格式动作 |
| P1-D 至 P1-F | 未实现 | 选区语义保护、知识库真实检索和导出一致性验收仍按本清单后续推进 |
