...
|---|---|---|---|---|
| 文稿 | **Web**：`WebDocumentWorkbench`；**Electron**：`DocumentEngineHost` | `web.docx.create` + export skills | Web 第一版 skill 驱动 | 见下文 Web Document Skills |
...
- `freewrite` → **Web** `WebDocumentWorkbench`；**Electron** `DocumentEngineHost`  
...
- `WebDocumentWorkbench`：顶栏（标题、模板、知识库、资料、导出）+ `A4RichEditor`（TipTap）+ `AICommandBox`。
- `A4RichEditor` 暴露 `getHtml` / `getSelection*` / `replaceSelection` / `insertAtCursor` / `replaceDocument`；页眉页脚与 `pageSpec` 来自当前 template skill。
...
2. 生成初稿：`web.document.generate` → `data.patch.replace_document` + `documentSession` + artifact。
3. AI 编辑：`web.document.edit`（`rewrite_selection` | `insert_at_cursor` | `replace_document` | `polish_document`）→ `data.patch` 由 `applyWebDocumentPatch` 写入编辑器。
