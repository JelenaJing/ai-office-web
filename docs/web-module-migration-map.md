...
|---|---|---|---|
| `document-generator` | AI 生成正文 | `document.generator.office_draft` | `web.docx.create` |
...
| `document-transformer` | 改写/优化 | （预留） | 后续 |

**数据流：**

1. `useWebDocumentSkills`：合并 `webDocumentBuiltInSkills` + `platformApi.skills.list()`（有 manifest 时追加）。
2. 生成：`generator.mapsToSkillId` → 返回 `data.documentSession` + `html` + `markdown` + artifact。
