...
interface ResultDisplayProps {
  ...
  onBatchInsertReferences?: (paperMarkdown: string) => Promise<void>;
}

export default function ResultDisplay({ result, onUseForVisualization, onRecipeFromOperations, onBatchInsertReferences }: ResultDisplayProps) {
  ...
  const getCoreResult = (data: any) => {
    ...
    // 全文 CoRemake（流式或 JSON）
    if (data._fullPaper) {
      return {
        type: 'full-paper-coremake',
        data: {
    ...
    // 如果是实验可视化结果
    if (data.extract && data.visualize) {
      return {
    ...
    if (Array.isArray(data.literature_pool) && (data.remade_introduction !== undefined || data.original_introduction !== undefined)) {
      return {
        ...
        data: {
          ...
          timeline_figure: data.timeline_figure || null,
        },
      };
    }
  ...
  if (!coreResult) {
    return (
      <div className="result-display-inner">
  ...
  switch (coreResult.type) {
    case 'idea':
      coreContent = <IdeaDisplay ideas={coreResult.data} />;
      break;
    case 'check':
      coreContent = <ContentCheckDisplay data={coreResult.data} onBatchInsertReferences={onBatchInsertReferences} />;
      break;
    case 'experiment':
      coreContent = <ExperimentDisplay data={coreResult.data} />;
      break;
    case 'experiment-extract':
      coreContent = <ExperimentExtractDisplay data={coreResult.data} onUseForVisualization={onUseForVisualization} />;
      break;
    case 'experiment-pipeline':
      coreContent = (
        <div className="result-default">
          <div style={{ marginBottom: '0.75rem' }}>
            <strong>🔗 实验链路结果（提取 → 可视化）</strong>
            <div style={{ marginTop: '0.25rem', color: '#64748b', fontSize: '0.85rem' }}>
              提示：上半部分是提取的步骤文本；下半部分是 DeepSyn 可视化（2D/3D 切换）。
            </div>
          </div>
          <div style={{ display: 'grid', gap: '0.75rem' }}>
            <div style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: 12, background: '#fff' }}>
              <ExperimentExtractDisplay data={coreResult.data.extract} onUseForVisualization={undefined} />
            </div>
            <div style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: 12, background: '#fff' }}>
              <ExperimentVisualizeDisplay data={coreResult.data.visualize} onRecipeFromOperations={onRecipeFromOperations} />
            </div>
          </div>
        </div>
      );
    ...
    case 'experiment-visualize':
      coreContent = <ExperimentVisualizeDisplay data={coreResult.data} onRecipeFromOperations={onRecipeFromOperations} />;
      break;
    case 'experiment-recipe':
      coreContent = <ExperimentRecipeDisplay data={coreResult.data} />;
      break;
    case 'theory':
      coreContent = <TheoryDisplay data={coreResult.data} />;
      break;
    case 'overall':
      coreContent = <OverallCheckDisplay data={coreResult.data} />;
      break;
    case 'introduction-remake':
      coreContent = <IntroductionRemakeDisplay data={coreResult.data} />;
      break;
    case 'full-paper-coremake':
      coreContent = <FullPaperCoRemakeDisplay data={coreResult.data} />;
      break;
    case 'plot':
      coreContent = <PlotDisplay data={coreResult.data} />;
      break;
    default:
      coreContent = <DefaultDisplay data={coreResult.data} />;
  }

  return <div className="result-display-inner">{coreContent}</div>;
}
...
function PlotDisplay({ data }: { data: any }) {
  ...
  const typeLine =
    meta.resolved_data_type || meta.resolved_template_id || meta.template_match_reason
      ? [
          meta.resolved_data_type ? `识别类型：${meta.resolved_data_type}` : null,
          meta.resolved_template_id ? `模板：${meta.resolved_template_id}` : null,
          meta.template_match_reason ? `说明：${meta.template_match_reason}` : null,
        ]
          .filter(Boolean)
          .join(' · ')
      : null;
  return (
    <div className="result-default">
      <div style={{ marginBottom: '0.75rem' }}>
        <strong>🎨 绘图结果</strong>
        {typeLine && (
          <div style={{ marginTop: '0.35rem', color: '#475569', fontSize: '0.85rem', lineHeight: 1.5 }}>
            {typeLine}
          </div>
        )}
        {data.plot_path && (
...
// 内容检查展示组件
function ContentCheckDisplay({ data, onBatchInsertReferences }: { data: any; onBatchInsertReferences?: (paperMarkdown: string) => Promise<void> }) {
  ...
  const referenceTargetText = (data.updated_text && data.updated_text.trim()) ? data.updated_text : originalText;
            ...
            <div className="text-comparison">
              <div className="text-original">
                ...
                <div className="text-content">
                  <RichText text={String(originalText)} />
                </div>
              </div>
              ...
              <div className="text-updated">
                ...
                <div className="text-content">
                  <RichText text={String(data.updated_text)} />
                </div>
              </div>
            </div>
          )}
      ...
      {hasReferences && (
        <div className="check-section">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            ...
            {onBatchInsertReferences && (
              <button
                type="button"
                className="link-btn"
                disabled={inserting || !referenceTargetText}
                onClick={async () => {
                  if (!referenceTargetText) {
                    alert('没有可用于插入 reference 的正文');
                    return;
                  }
                  setInserting(true);
                  try {
                    await onBatchInsertReferences(referenceTargetText);
                  } finally {
                    setInserting(false);
                  }
                }}
                title="读取当前全文并自动搜集/插入 reference"
              >
                {inserting ? '处理中…' : '一键批量插入 Reference'}
              </button>
            )}
          </div>
...
function ExperimentDisplay({ data }: { data: any }) {
  const design = data?.experiment_design && typeof data.experiment_design === 'object' ? data.experiment_design : {};
  return (
    <div className="result-experiment">
        ...
        <div className="experiment-section">
          ...
          <div className="experiment-design">
            {design.purpose && (
              <div>
                <strong>目的：</strong>
                <RichText text={String(design.purpose)} />
              </div>
            )}
            {design.principle && (
              <div>
                <strong>原理：</strong>
                <RichText text={String(design.principle)} />
              </div>
            )}
            {design.method && (
              <div>
                <strong>方法：</strong>
                <RichText text={String(design.method)} />
              </div>
            )}
            {design.expected_results && (
              <div>
                <strong>预期结果：</strong>
                <RichText text={String(design.expected_results)} />
              </div>
            )}
          </div>
        </div>
      ...
      {data.recipe && (
        <div className="experiment-section">
          <strong>实验配方：</strong>
          <div className="experiment-recipe">
            <RichText text={String(data.recipe)} />
          </div>
        </div>
      )}
    </div>
...
function MarkdownWithMath({ text }: { text: string }) {
  ...
  try {
    ...
    return <>{renderTextWithFormulas(normalized)}</>;
  }
}

function RichText({ text, className = '' }: { text: string; className?: string }) {
  return (
    <div className={`result-rich-text ${className}`.trim()}>
      <MarkdownWithMath text={String(text || '')} />
    </div>
  );
}
...
function TheoryDisplay({ data }: { data: any }) {
  const formulasAsMarkdown = (Array.isArray(data.formulas) ? data.formulas : [])
    .map((formula: string) => {
    ...
    .filter(Boolean)
    .join('\n\n');
  const stepsAsMarkdown = (Array.isArray(data.derivation_steps) ? data.derivation_steps : [])
    .map((step: string, idx: number) => `${idx + 1}. ${String(step || '').trim()}`)
    .join('\n');
      ...
      {data.analysis && (
        <div className="theory-section">
          ...
          <div className="theory-analysis">
            <MarkdownWithMath text={String(data.analysis)} />
          </div>
        </div>
      )}
      {data.formulas && data.formulas.length > 0 && (
        <div className="theory-section">
          ...
          <div className="theory-formulas">
            <MarkdownWithMath text={formulasAsMarkdown} />
          </div>
        </div>
      )}
      {data.derivation_steps && data.derivation_steps.length > 0 && (
        <div className="theory-section">
          ...
          <div className="theory-steps">
            <MarkdownWithMath text={stepsAsMarkdown} />
          </div>
        </div>
      )}
    </div>
...
function FullPaperCoRemakeDisplay({ data }: { data: any }) {
  const errs = Array.isArray(data.errors) ? data.errors : [];
  const jobs = data.parallel_jobs && typeof data.parallel_jobs === 'object' ? data.parallel_jobs : {};
  const streamSection =
    typeof data._streamingSection === 'string' && data._streamingSection ? data._streamingSection : null;
  return (
    <div className="result-full-paper-coremake" style={{ fontSize: '0.9rem', lineHeight: 1.55 }}>
      {data._streaming && (
        <div className="result-streaming-banner">
          <div className="remake-spinner" aria-hidden="true" />
          <span>
      ...
      <div style={{ marginBottom: '0.75rem', color: '#64748b', fontSize: '0.85rem' }}>
        标题格式为 <code>[CoRemake] 原标题</code>；含 Abstract、Introduction、Methods、Results、Theory、Conclusion、References。
      </div>
      {Object.keys(jobs).length > 0 && (
        <details style={{ marginBottom: '0.75rem' }}>
          <summary style={{ cursor: 'pointer', fontWeight: 600 }}>并行子任务状态</summary>
          <pre
            style={{
              marginTop: 8,
              padding: 10,
      ...
      {errs.length > 0 && (
        ...
        >
          <strong>降级 / 告警（{errs.length}）</strong>
          <ul style={{ margin: '0.35rem 0 0 1rem', padding: 0 }}>
            {errs.map((e: any, i: number) => (
              <li key={i}>
                <code>{String(e?.stage || 'unknown')}</code>: {String(e?.error || '')}
              </li>
            ))}
          </ul>
        </div>
      ...
      {data.markdown ? (
        <div className="full-paper-markdown" style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: 12, background: '#fff' }}>
          <MarkdownWithMath text={String(data.markdown)} />
        </div>
      ) : (
        !data._streaming && <div style={{ color: '#94a3b8' }}>暂无正文（请查看上方错误信息）</div>
      )}
    </div>
  );
}
    ...
    <div className="result-introduction-remake" style={{ fontSize: '0.9rem', lineHeight: 1.55 }}>
      ...
      {data.continuity_notes && (
        <div style={{ marginBottom: '0.75rem', padding: '0.5rem', background: '#f1f5f9', borderRadius: '6px' }}>
          ...
          <div style={{ marginTop: '0.25rem' }}>
            <RichText text={String(data.continuity_notes)} />
          </div>
        </div>
      )}
      ...
      {data.original_introduction && (
        <details style={{ marginBottom: '0.75rem' }}>
          ...
          <div style={{ margin: '0.5rem 0 0', fontSize: '0.82rem' }}>
            <RichText text={String(data.original_introduction)} />
          </div>
        </details>
      )}
      ...
      {data.remade_introduction && (
        <div style={{ marginBottom: '0.75rem' }}>
          ...
          <div style={{ marginTop: '0.35rem' }}>
            <RichText text={String(data.remade_introduction)} />
          </div>
        </div>
      )}
              ...
              return (
                <li key={i} style={{ marginBottom: '0.35rem' }}>
                  ...
                  {r.relevance_reason && (
                    <>
                      {' '}
                      <strong>{r.relevance_reason}</strong>.
                    </>
                  )}
                  {r.pool_index != null && (
      ...
      {data.timeline_figure && data.timeline_figure.image_file_url && (
        <div style={{ marginTop: '0.75rem' }}>
          <strong>领域发展 Timeline</strong>
          {data.timeline_figure.overall_description && (
          ...
          <div style={{ marginTop: '0.5rem' }}>
            <img
              src={data.timeline_figure.image_file_url}
              alt="Field development timeline"
              style={{ maxWidth: '100%', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}
            />
          </div>
          {data.timeline_figure.detail_description && (
            <div style={{ marginTop: '0.35rem', fontSize: '0.82rem', color: '#64748b', fontStyle: 'italic' }}>
              {data.timeline_figure.detail_description}
            </div>
          )}
        </div>
      ...
      {data.summary && (
        <div className="extract-summary">
          ...
          <div className="summary-text">
            <RichText text={String(data.summary)} />
          </div>
        </div>
      )}
      {data.experiment_text && (
        <div className="extract-steps">
          ...
          <div className="steps-text">
            <RichText text={String(data.experiment_text)} />
          </div>
        </div>
      )}
              ...
              <li key={idx} className="section-item">
                ...
                {section.content && (
                  <div className="section-content">
                    <RichText text={String(section.content)} />
                  </div>
                )}
              </li>
            ))}
