"""
并入现有 paper-remake-service 时：在 app/main.py 中增加一行（若尚未注册 domain）：
    from app.routers.remake_domain_only import router as domain_only_router
    app.include_router(domain_only_router)

注意：若 remake.py 已注册同名 POST /domain，请勿重复 include，应直接改 remake.py 或二选一。

本文件依赖 app.models 中的 DomainGenerateRequest、DomainGenerateResponse 及
app.agents.domain_generator、app.services.draw_gateway_client、app.project_manager。
"""
import logging
